...
var addItem = function(anItemArray, item)
{
  for (var i = 0; i < anItemArray.length; i++)
  {
    if (anItemArray[i].name === item.name)
    {
      ...
      console.log(anItemArray[i].count);
      return;
...
var decideWhatToAdd = function ()
{
  ...
  {
    alert(userInputUpper + " is not a valid value! Goodbye.")
    decideWhatToAdd();
  }
}
...
var decideToRunProgram = function()
{
  ...
  {
    alert("It seems you did not put in a real value. Goodbye!");  // Doesn't make run equal true, so the cartManager never gets called.
  }
  return run;
}
